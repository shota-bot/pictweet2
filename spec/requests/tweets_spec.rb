...
describe TweetsController, type: :request do
  ...
  describe "GET #index" do
    it "indexアクションにリクエストを送ると正常にレスポンスが返ってくる" do
      get root_path
      expect(response.status).to eq 200
    end
    it "indexアクションにリクエストを送るとレスポンスに投稿済みのツイートのテキストが存在する" do
      get root_path
      expect(response.body).to include @tweet.text
    end
    it "indexアクションにリクエストを送るとレスポンスに投稿済みのツイートの画像urlが存在する" do
      get root_path
      expect(response.body).to include @tweet.image
    end
    it "indexアクションにリクエストを送るとレスポンスに投稿検索フォームが存在する" do
      get root_path
